...
func (m *metric) Split(maxSize int) []asgard.Metric {
	if m.Len() <= maxSize {
		return []asgard.Metric{m}
	}
	var out []asgard.Metric
...
func (m *metric) Copy() asgard.Metric {
	return copyWith(m.name, m.tags, m.fields, m.t)
}

func copyWith(name, tags, fields, t []byte) asgard.Metric {
	out := metric{
...
func New(
	...
	var thisType asgard.ValueType
	if len(mType) > 0 {
		thisType = mType[0]
	} else {
		thisType = asgard.Untyped
	}
