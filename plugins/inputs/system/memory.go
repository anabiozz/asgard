...
func (s *MemStats) Gather(acc asgard.Accumulator) error {
	vm, err := s.ps.VMStat()
	if err != nil {
...
func (s *SwapStats) Gather(acc asgard.Accumulator) error {
...
func init() {
	ps := newSystemPS()
	inputs.Add("mem", func() asgard.Input {
		return &MemStats{ps: ps}
	})

	inputs.Add("swap", func() asgard.Input {
