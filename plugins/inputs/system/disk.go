...
func (s *DiskStats) Gather(acc asgard.Accumulator) error {
	// Legacy support:
...
func (s *DiskIOStats) Gather(acc asgard.Accumulator) error {
...
func init() {
	ps := newSystemPS()
	inputs.Add("disk", func() asgard.Input {
		return &DiskStats{ps: ps}
	})

	inputs.Add("diskio", func() asgard.Input {
